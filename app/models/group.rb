class Group < ActiveRecord::Base
  ...
  def add_activity
    ...
    user.activities.create(:activity_type => 'group', 
      :text => "<a href='/user/#{self.user.username}'>#{self.user.formatted_name}</a> created a new group - <a href='/groups/#{self.id}'>#{self.name}</a>.")
  end  
end
